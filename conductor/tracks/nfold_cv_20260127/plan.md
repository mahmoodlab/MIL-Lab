...
## Phase 1: Split Generation Infrastructure [checkpoint: 05bb0fe]
- [x] Task: Create Split Generation Script `scripts/create_splits.py`
    - [x] Sub-task: Implement logic to read the main labels CSV and group by `case_id`.
    - [x] Sub-task: Use `sklearn.model_selection.KFold` or `StratifiedKFold` to generate K partitions of unique `case_id`s.
    - [x] Sub-task: For each fold, further split the "training" portion into `train` and `val` sets based on a configurable ratio.
    - [x] Sub-task: Implement a JSON export function that saves the mapping of `case_id` to its set (`train`, `val`, `test`).
    - [x] Sub-task: Ensure the script accepts command-line arguments for `n_folds`, `val_ratio`, `seed`, and `label_csv`.
- [x] Task: Write Tests for Split Logic `tests/test_splitting.py`
    - [x] Sub-task: Verify that no `case_id` overlaps between sets in a single fold.
    - [x] Sub-task: Verify that all slides for a given `case_id` are assigned to the same set.
