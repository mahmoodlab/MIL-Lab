#!/usr/bin/env python3
"""
Standalone script to train CLAM on PANDA dataset
...
class PANDAH5Dataset(Dataset):
    """Custom dataset for PANDA with UNI v2 features (CLAM version)"""
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(feat_path, "r") as f:
            ...
            if len(raw_features.shape) == 3:
                ...
                features = raw_features.squeeze(0)
            else:
...
# Create CLAM model
print("Creating CLAM model...")
model = create_model(
    'clam.base_ce.uni_v2.none',  # CLAM with CE loss, no pretrained weights
...
# Create dataloaders (CLAM requires batch_size=1)
...
print("Confusion matrix saved to: confusion_matrix_clam_panda.png")
