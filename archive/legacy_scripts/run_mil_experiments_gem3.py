...
Corrected to align with 'Do Multiple Instance Learning Models Transfer?' replication details.
"""
...
# NOTE: Ensure 'create_dataloaders' implements a WeightedRandomSampler for class imbalance [cite: 852]
# NOTE: Ensure 'preprocess_panda_data' uses specific UNI patient splits, NOT random splits
...
def train_and_evaluate_model(
    ...
    # Create dataloaders
    # IMPORTANT: Ensure train_loader uses WeightedRandomSampler internally [cite: 852]
    ...
    best_val_kappa = -1.0  # FIX: Initialize kappa tracker to worst possible score (-1.0)
    patience_counter = 0
    train_losses = []
    val_losses = []
    val_accuracies = []
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for features, labels in tqdm(train_loader, desc=f'Epoch {epoch+1}/{NUM_EPOCHS} [Train]'):
            ...
            # FIX: Removed F.normalize. Features should maintain magnitude for attention mechanisms.
        ...
        with torch.no_grad():
            for features, labels in tqdm(val_loader, desc=f'Epoch {epoch+1}/{NUM_EPOCHS} [Val]'):
                ...
                # FIX: Removed F.normalize here as well.
                # Validation features must match training distribution (minus dropout).
        ...
        # Early Stopping & Checkpointing Logic [cite: 850]
        # PANDA uses Kappa as the primary metric, so we save based on best Kappa, not Loss.
